...
calc_nrm <- function(cohort, days) {
  ...
  n_nrm <- sum(cohort$death_type == "NRM" & cohort$days_to_event <= days)
...
calc_relapse <- function(cohort, days) {
  ...
  n_relapse <- sum(cohort$death_type == "Relapse" & cohort$days_to_event <= days)
...
# GVHD by donor type
major_donors <- c("Sibling HLA-identical", "Haploidentical", "MUD")
...
ggplot(plot_data, aes(x = timepoint, y = pct, fill = outcome)) +
  ...
  labs(title = "Figure 1: Cumulative Mortality Outcomes by Timepoint",
       subtitle = "Cumulative Rolling Cohort Analysis - 2025 (NRM = Non-Relapse Mortality)",
...
if (!is.na(overall_mort$n_events[3]) && overall_mort$n_events[3] > 0) {
  ...
  cat(paste0("- Relapse accounts for **", relapse_prop, "%** of all deaths at 365 days\n"))
}
```
...
### 7.1 Overall Mortality by Donor Type and CR Status (365 days)

```{r mort-donor-cr-365d}
...
for (cr_status in c("Complete Remission", "Not in Complete Remission")) {
  ...
  for (donor in major_donors) {
    ...
    if (nrow(cohort_365d_donor_cr) > 0) {
      mort_result <- calc_mortality(cohort_365d_donor_cr, 365) |>
...
if (nrow(mort_donor_cr_strat) > 0) {
  kable(mort_donor_cr_strat |>
    ...
    select(`Disease Status` = cr_status, `Donor Type` = donor,
           `Cohort Size` = n_cohort, Deaths = n_events, `Mortality Rate` = display),
        caption = "Table 16: 365-Day Mortality by Donor Type and Disease Status") |>
    kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE) |>
    pack_rows("Complete Remission", 1, sum(mort_donor_cr_strat$cr_status == "Complete Remission"), 
              label_row_css = "background-color: #27AE60; color: white;") |>
    pack_rows("Not in Complete Remission", 
              sum(mort_donor_cr_strat$cr_status == "Complete Remission") + 1, nrow(mort_donor_cr_strat),
...
### 7.2 NRM by Donor Type and CR Status (365 days)
...
for (cr_status in c("Complete Remission", "Not in Complete Remission")) {
  ...
  for (donor in major_donors) {
    ...
    if (nrow(cohort_365d_donor_cr) > 0) {
      nrm_result <- calc_nrm(cohort_365d_donor_cr, 365) |>
        mutate(cr_status = cr_status, donor = donor)
...
if (nrow(nrm_donor_cr_strat) > 0) {
  kable(nrm_donor_cr_strat |>
    ...
    select(`Disease Status` = cr_status, `Donor Type` = donor,
           `Cohort Size` = n_cohort, `NRM Events` = n_events, `NRM Rate` = display),
        caption = "Table 17: 365-Day NRM by Donor Type and Disease Status") |>
    kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE) |>
    pack_rows("Complete Remission", 1, sum(nrm_donor_cr_strat$cr_status == "Complete Remission"),
              label_row_css = "background-color: #27AE60; color: white;") |>
    pack_rows("Not in Complete Remission",
              sum(nrm_donor_cr_strat$cr_status == "Complete Remission") + 1, nrow(nrm_donor_cr_strat),
              label_row_css = "background-color: #E74C3C; color: white;")
}
```

### 7.3 Relapse Mortality by Donor Type and CR Status (365 days)

```{r relapse-donor-cr-365d}
...
for (cr_status in c("Complete Remission", "Not in Complete Remission")) {
  ...
  for (donor in major_donors) {
    ...
    if (nrow(cohort_365d_donor_cr) > 0) {
      relapse_result <- calc_relapse(cohort_365d_donor_cr, 365) |>
        mutate(cr_status = cr_status, donor = donor)
...
if (nrow(relapse_donor_cr_strat) > 0) {
  kable(relapse_donor_cr_strat |>
    ...
    select(`Disease Status` = cr_status, `Donor Type` = donor,
           `Cohort Size` = n_cohort, `Relapse Deaths` = n_events, `Relapse Rate` = display),
        caption = "Table 18: 365-Day Relapse Mortality by Donor Type and Disease Status") |>
    kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE) |>
    pack_rows("Complete Remission", 1, sum(relapse_donor_cr_strat$cr_status == "Complete Remission"),
              label_row_css = "background-color: #27AE60; color: white;") |>
    pack_rows("Not in Complete Remission",
              sum(relapse_donor_cr_strat$cr_status == "Complete Remission") + 1, nrow(relapse_donor_cr_strat),
              label_row_css = "background-color: #E74C3C; color: white;")
}
```

### 7.4 Acute GVHD by Donor Type and CR Status (100 days)
...
if (nrow(cgvhd_donor_cr_strat_any) > 0) {
  cat("#### Chronic GVHD (Any) by Donor Type and CR Status\n\n")
  kable(cgvhd_donor_cr_strat_any |>
    mutate(display = paste0(pct, "% (", n_events, "/", n_cohort, ")")) |>
    select(`Disease Status` = cr_status, `Donor Type` = donor,
           `Cohort Size` = n_cohort, Events = n_events, `Chronic GVHD (Any) Rate` = display),
        caption = "Table 20: 365-Day Chronic GVHD (Any) by Donor Type and Disease Status") |>
    kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE) |>
  ...
  kable(cgvhd_donor_cr_strat_modsev |>
    mutate(display = paste0(pct, "% (", n_events, "/", n_cohort, ")")) |>
    select(`Disease Status` = cr_status, `Donor Type` = donor,
           `Cohort Size` = n_cohort, Events = n_events, `Chronic GVHD (Mod-Severe) Rate` = display),
        caption = "Table 21: 365-Day Chronic GVHD (Moderate-Severe) by Donor Type and Disease Status") |>
    kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE) |>
...
3. **NRM vs Relapse at 365 days**: `r nrm_overall$pct[3]`% (NRM) vs `r relapse_overall$pct[3]`% (Relapse)
