...
calc_trm <- function(cohort, days) {
  ...
  n_trm <- sum(cohort$death_type == "TRM" & cohort$days_to_event <= days)
...
### Mortality by Cause: TRM, NRM, and Relapse
...
kable(cause_summary,
      caption = "Table 2: Cumulative Mortality Breakdown by Cause (Overall = TRM + NRM + Relapse)") |>
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE) |>
...
At 365 days: **TRM = `r trm_overall$pct[3]`%**, **NRM = `r nrm_overall$pct[3]`%**, and **Relapse = `r relapse_overall$pct[3]`%**
:::
...
plot_data <- bind_rows(
  ...
  mutate(
    ...
    outcome = factor(outcome, levels = c("Overall Mortality", "TRM", "NRM", "Relapse"))
  )
...
ggplot(plot_data, aes(x = timepoint, y = pct, fill = outcome)) +
  ...
  labs(title = "Figure 1: Cumulative Mortality Outcomes by Timepoint",
       subtitle = "Cumulative Rolling Cohort Analysis - 2025 (TRM, NRM, Relapse)",
