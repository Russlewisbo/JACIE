...
transplant_clean <- transplant_data |>
  mutate(
    ...
    # Competing risks using death_type from dataset
    # 0 = Alive (censored), 1 = TRM, 2 = NRM, 3 = RRM (Relapse)
...
-   Mortality analysis: TRM, NRM, and RRM (Relapse-Related Mortality)
...
-   TRM (n=4) and NRM (n=2) together account for 67% of deaths; RRM accounts for 33% (n=3)
...
The plot is restricted to 375 days to avoid misleading interpretation. Only 1 of 13 "Not in CR" patients has follow-up beyond 400 days, which would make survival estimates unreliable at later timepoints.
:::
...
# Plot 1: Overall cumulative incidence
plot(cif_overall, 
     main = "Cumulative Incidence: Overall (n=47)",
     xlab = "Days post-HSCT",
     ylab = "Cumulative Incidence",
     col = c("#E74C3C", "#9B59B6", "#3498DB"),
     lwd = 2.5,
     xlim = c(0, 360),
     ylim = c(0, 0.20),
     wh = c(-100, -100),
     cex.main = 1.2,
     cex.lab = 1.0)
abline(v = c(30, 100), lty = 3, col = "gray60")
legend("topright", 
       legend = c("TRM (n=4)", "NRM (n=2)", "RRM (n=3)"),
       col = c("#E74C3C", "#9B59B6", "#3498DB"),
       lty = 1, lwd = 2.5, bty = "n", cex = 0.8)

# Plot 2: By disease status (CR vs Not in CR)
plot(cif_disease,
     main = "Cumulative Incidence by Disease Status",
     xlab = "Days post-HSCT",
     ylab = "Cumulative Incidence",
     col = c("#27AE60", "#E74C3C", "#8E44AD", "#F39C12", "#2980B9", "#1ABC9C"),
     lwd = 2.5,
     xlim = c(0, 360),
     ylim = c(0, 0.25),
     wh = c(-100, -100),
...
abline(v = c(30, 100), lty = 3, col = "gray60")
legend("topright", 
       legend = c("CR: TRM", "Not CR: TRM", 
                  "CR: NRM", "Not CR: NRM",
                  "CR: RRM", "Not CR: RRM"),
       col = c("#27AE60", "#E74C3C", "#8E44AD", "#F39C12", "#2980B9", "#1ABC9C"),
       lty = 1, lwd = 2, bty = "n", cex = 0.65, ncol = 1)
...
```{r mortality-tables}
# Calculate actual mortality counts by death_type
mortality_counts <- transplant_clean |>
  filter(death == 1) |>
  count(death_type) |>
  pivot_wider(names_from = death_type, values_from = n, values_fill = 0)

mortality_summary <- tibble(
  Death_Type = c("TRM (Treatment-Related Mortality)", 
                 "NRM (Non-Relapse Mortality)", 
                 "RRM (Relapse-Related Mortality)"),
  N = c(sum(transplant_clean$death_type == "TRM", na.rm = TRUE),
        sum(transplant_clean$death_type == "NRM", na.rm = TRUE),
        sum(transplant_clean$death_type == "RRM", na.rm = TRUE)),
  Percent = round(N / nrow(transplant_clean) * 100, 1)
)

kable(mortality_summary,
      col.names = c("Death Type", "n", "%"),
      caption = "Mortality by Type (Overall Cohort, n=47)") |>
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
plot_data <- transplant_clean |>
  ...
  mutate(
    label_text = paste0(n, " (", round(percentage, 0), "%)"),
    # Convert donor_main to numeric for positioning
    x_num = as.numeric(factor(donor_main, levels = c("Haplo", "MUD", "Sibling"))),
    # Left edge inside the bar (bar width is 0.7, so left edge is x - 0.35, add small padding)
    x_label = x_num - 0.32
  )

ggplot(plot_data, aes(x = donor_main, y = percentage, fill = acute_gvhd_detailed)) +
  geom_col(width = 0.7) +
  # Labels left-justified inside each bar
  geom_text(aes(x = x_label, label = label_text),
...
chronic_plot_data <- transplant_clean |>
  ...
  ungroup() |>
  # Create labels and calculate positions for left-justified labels inside bars
  mutate(
    label_text = paste0(n, " (", round(percentage, 0), "%)"),
    # Convert donor_main to numeric for positioning
    x_num = as.numeric(factor(donor_main, levels = c("Haplo", "MUD", "Sibling"))),
...
3.  **TRM and NRM are the predominant causes of death**
    -   TRM: 4 deaths (44% of all deaths)
    -   NRM: 2 deaths (22% of all deaths)
    -   RRM (Relapse): 3 deaths (33% of all deaths)
...
4.  **Relapse mortality only affects patients not in remission**
    ...
    -   All 3 RRM deaths occurred in patients NOT in CR at transplant
...
-   **Competing Risks**: Cumulative incidence functions for TRM, NRM, and RRM (relapse mortality)
